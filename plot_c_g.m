...
for exp_idx = 1:length(exp_x_vals)
    ...
    for sub_idx = 1:length(diff_data)
        ...
        % set x vals
        x_vals = (1:length(patterns)) + jitter_dots(sub_idx);

        %% Plot Average
        plot_avg = ...
            plot(ax, x_vals, avg_data, ...
            "LineStyle", "none", ...
            "LineWidth", linewidth, ...
            "Marker", "o", ...
            "Color", avg_colours, ...
        ...
        %% Plot Error
        err_plot = ...
            errorbar(ax, x_vals, avg_data, err_data_low, err_data_up, ...
            "LineStyle", "none", ...
            "Color", avg_colours, ...
            "LineWidth", linewidth, ...
            "CapSize", capsize, ...
...
errr_plot = errorbar(ax, -10, 1, 1, 1, ...
    "LineStyle", "none", ...
    "LineWidth", linewidth, ...
    "Color", axis_colour, ...
    "CapSize", capsize, ...
...
if length(diff_data) > 1    % Plot the crows
    ...
    % Adjust Colors for Error & Mean Plot
    avg_colours = colours_pattern_diff{2};
    avg_colours = rgb2hsv(avg_colours);
    avg_colours(:, 3) = avg_colours(:, 3) * .7;
    avg_colours = hsv2rgb(avg_colours);
    % Add invisible Uri plot
    uri_plot = ...
        boxchart(ax, NaN(8, 1), NaN(8, 1), ...
