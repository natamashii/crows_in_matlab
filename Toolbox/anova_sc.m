function statistics_s_c = anova_sc(performances_s, performances_c, ...
...
for pattern = 1:length(patterns)
    ...
    for sample_idx = 1:size(numerosities, 1)
        ...
        for cond_idx = 1:2
            ...
            % condition & data cols
            if cond_idx == 1    % Standard Conditions
                condition_col = repmat({1}, ...
                    size(subjects, 1), 1);
                performance_col = ...
                ...
                for sub_idx = 1:size(rec_times_s, 1)
                    rec_times_col(sub_idx) = ...
                        median(vertcat(rec_times_s{sub_idx, pattern, sample_idx, 1}), "omitnan");
                end

            elseif cond_idx == 2    % Control Conditions
                condition_col = repmat({2}, ...
                    size(subjects, 1), 1);
                performance_col = ...
                ...
                for sub_idx = 1:size(rec_times_c, 1)
                    rec_times_col(sub_idx) = ...
                        median(vertcat(rec_times_c{sub_idx, pattern, sample_idx, 1}), "omitnan");
                end
            end
    ...
    data_table.Condition = grp2idx(data_table.Condition);
...
statistics_s_c.ResponseFrequency = ...
...
save([adapt_path filename '.mat'], '-struct', 'statistics_s_c')
