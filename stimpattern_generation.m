...
rad_back = 1;  % radius for x-axis (1. dim) and y-axis (2. dim) in []
        ...
        while ~check
            ...
            switch pattern_type
                ...
                case "P2"   % additive
                    if curr_num <= 1
                        dot_pos_limit = max(max(x * rad_back, y * rad_back)) ...
                            - 2 * (dot_radii * scaling) * 1;
                        dot_pos = (2 * dot_pos_limit) * (rand(2, 1) - .5);
                        break
                    else
                        group_check = false;
                        % set how dots should be grouped
